    ...
    <div className="grid min-h-0 gap-2">
      ...
      <Panel title="Internal USB Grid" titleClassName="text-cyan-300" className="p-2">
          ...
          {containedSlots.map((slot) => {
            ...
            return (
              <button
                key={slot.slotId}
                type="button"
                title={`Move ${slot.item.name} to stash`}
                onClick={() => onRemoveUsb(slot.slotId)}
                className="relative z-10 overflow-hidden border border-cyan-500/55 bg-cyan-500/10 p-1 active:scale-[0.98]"
                style={{
        ...
        <p className="mt-2 text-center text-[7px] font-black uppercase text-zinc-600">
          Tap a stored USB to return it to the main stash
        </p>
      </Panel>
